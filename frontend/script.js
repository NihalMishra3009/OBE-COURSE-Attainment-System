...
function printCIAQPaper(tIdx){
  ...
  html+='</tbody></table>';
  html+='<p style="margin-top:20px;font-size:11px;color:#666">Generated by OBE Attainment System | '+new Date().toLocaleDateString('en-IN')+'</p>';
  html+='</body></html>';
...
function renderCertificate(el){
  ...
  <div id="certPrint" style="background:#fff;border:3px solid #d97706;border-radius:16px;max-width:780px;margin:0 auto;padding:40px 38px 24px;position:relative;overflow:hidden;min-height:760px;box-sizing:border-box">
    ...
    <div style="position:relative;z-index:1;display:flex;flex-direction:column;align-items:center;justify-content:flex-start;text-align:center">
      <div style="display:flex;align-items:center;justify-content:center;gap:18px;margin-bottom:16px;width:100%">
      ...
      <div style="font-size:13px;color:#64748b;margin:8px 0 18px;letter-spacing:1px">Outcome Based Education &#8212; Attainment Documentation</div>
      <div style="width:76px;height:3px;background:linear-gradient(90deg,#d97706,transparent);margin:0 auto 22px"></div>
      <div style="font-size:14px;line-height:1.8;color:#334155;max-width:560px;margin:0 auto 22px">
...
function exportAllExcel(){
  const s=sub();
  const wb=XLSX.utils.book_new();
  XLSX.utils.book_append_sheet(wb,XLSX.utils.aoa_to_sheet([['OBE Attainment — '+s.name],[''],['Code',s.code],['Dept',s.dept],['Faculty',s.faculty],['AY',s.ay],['Sem',s.sem],['Target',s.coTargetLevel.toFixed(2)]]),'1_CourseInfo');
  ...
  XLSX.writeFile(wb,'OBE_'+s.code+'_'+s.ay+'.xlsx');

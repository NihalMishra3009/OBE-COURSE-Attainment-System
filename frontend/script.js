...
function renderCertificate(el){
  const s=sub();
  const achieved=s.coAttainment.filter(c=>c&&c.achieved).length;
  const total=s.cos.length;
  el.innerHTML='<div class="card" id="certPrint"><div class="card-body" style="text-align:center;padding:48px">'
    +'<div style="font-size:48px;margin-bottom:16px">🏅</div>'
    +'<div style="font-size:28px;font-weight:800;color:var(--accent)">NBA OBE Course File</div>'
    +'<div style="font-size:18px;font-weight:700;margin:8px 0;color:var(--text2)">Certificate of Completion</div>'
    +'<div style="font-size:14px;color:var(--text2);margin-bottom:24px">Academic Year '+s.ay+' | Semester '+s.sem+'</div>'
    +'<div style="font-size:22px;font-weight:800;margin-bottom:8px">'+s.name+'</div>'
    +'<div style="font-size:15px;color:var(--text2);margin-bottom:4px">Course Code: <strong>'+s.code+'</strong></div>'
    +'<div style="font-size:15px;color:var(--text2);margin-bottom:4px">Department: <strong>'+s.dept+'</strong></div>'
    +'<div style="font-size:15px;color:var(--text2);margin-bottom:24px">Faculty: <strong>'+s.faculty+'</strong></div>'
    +'<div class="kpi '+(achieved===total?'green':'gold')+'" style="display:inline-block;padding:20px 40px;margin-bottom:24px">'
    +'<div class="kpi-val" style="font-size:36px">'+achieved+'/'+total+'</div>'
    +'<div class="kpi-label">COs Achieved</div></div>'
    +'<div style="font-size:13px;color:var(--text3);margin-top:24px">Generated by NBA OBE Attainment System • '+new Date().toLocaleDateString('en-IN')+'</div>'
    +'</div></div>'
    +'<div style="margin-top:12px;display:flex;gap:8px;justify-content:center">'
    +'<button class="btn btn-primary" onclick="generateFullReport()">📄 Full Report</button>'
    +'<button class="btn btn-outline" onclick="exportAllExcel()">📊 Excel Export</button>'
    +'</div>';
}
